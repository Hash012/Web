...
export default function Report() {
  ...
  for (const f of newFiles) {
    ...
    try {
      ...
      if (res.ok && result.sheets && result.sheets.length > 0) {
        ...
        // 更新sheet名称
        const updatedSheets = result.sheets.map((sheet, idx) => ({
          ...sheet,
          name: `${fileBase}-Sheet${currentFileSheets.length + idx + 1}`
        }));
        
        allSheets = [...allSheets, ...updatedSheets];
        hasValidSheet = true;
      } else if (result.msg) {
        errorMsgs.push(`文件 ${f.name}: ${result.msg}`);
      } else {
        errorMsgs.push(`文件 ${f.name}: 未知错误`);
      }
    } catch (err) {
      errorMsgs.push(`文件 ${f.name}: 网络错误`);
    }
  }
  ...
  if (errorMsgs.length > 0) {
    setMsg(errorMsgs.join("；"));
    localStorage.setItem('report_msg', errorMsgs.join("；"));
  } else if (hasValidSheet) {
    setMsg("解析成功");
    localStorage.setItem('report_msg', "解析成功");
    setIsNewUpload(true);
  } else {
    setMsg("未识别到有效表格");
    localStorage.setItem('report_msg', "未识别到有效表格");
  }
